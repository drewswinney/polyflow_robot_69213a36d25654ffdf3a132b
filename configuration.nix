...
let
  user      = "admin";
  password  = "password";
  hostname  = "69213a36d25654ffdf3a132b";
  homeDir   = "/home/${user}";
  ...
  wifiModeSwitch = pkgs.writeShellApplication {
    ...
    runtimeInputs = [ pkgs.networkmanager pkgs.gawk pkgs.coreutils ];
    text = ''
      set -euo pipefail
      ...
      # Create/update client connection
      if ! nmcli -t -f NAME connection show | grep -Fx "robot-wifi" >/dev/null; then
        nmcli connection add type wifi ifname "$WIFI_IF" con-name robot-wifi ssid "$WIFI_SSID"
      else
        nmcli connection modify robot-wifi 802-11-wireless.ssid "$WIFI_SSID"
      fi
      ...
      if [ -n "''${WIFI_PSK:-}" ]; then
        ...
        nmcli connection modify robot-wifi 802-11-wireless-security.key-mgmt none
      fi

      nmcli connection modify robot-wifi ipv4.method auto ipv6.method auto connection.autoconnect yes

      echo "[wifi-mode] Bringing up STA connection to SSID=$WIFI_SSID" >&2
      nmcli connection down robot-ap || true
      nmcli connection up robot-wifi || true
    '';
  };
...
in
{
  ...
  services.nginx = {
    ...
    virtualHosts."default" = {
      ...
      locations = {
        ...
        "/api/".proxyPass = "http://127.0.0.1:8082/";
        "/" = {
  ...
  systemd.services.polyflow-robot-api = {
    ...
    environment = {
      ...
      ROBOT_API_ALLOWED_ORIGINS = "http://localhost,http://127.0.0.1,http://${hostname}.local";
    };
    serviceConfig = {
